...
每天定时运行一次（建议 08:00，本地时区，失败时通知），做两件事：
...
- 只处理自上次运行以来的增量：Codex 任务按"上次同步后有变化"筛选；笔记按 `/Users/sgx/Documents/Notes/4-Agents/memory/同步状态.md` 的 last_commit（上次运行结束时的 HEAD）之后的 git 变更筛选，辅以 `git status --porcelain` 兜底未提交改动。

## 0. 启动必读

- `/Users/sgx/Documents/Notes/4-Agents/memory/00-入口.md`
- `/Users/sgx/Documents/Notes/4-Agents/memory/同步状态.md`
- `/Users/sgx/Documents/Notes/4-Agents/memory/projects/项目索引.md`、`archive/归档索引.md`
- `/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/知识索引.md`（播放/工程知识索引）
...
每天整理 Codex 可访问任务中的新增沟通记录，维护 `/Users/sgx/Documents/Notes/4-Agents/memory`。当前只维护 Notes、wow_1、wow_2；
...
- 播放业务/工程事实（接口、协议、操作手册、踩坑＝问题/根因/修复）→ `/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/` 下对应子目录（`业务模块/`、`工程基建/`、`排障经验/`、`规则与纪律/`，按内容归类）。先查重：`/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/知识索引.md` + 目录检索；有同主题只增量更新对应文档和索引摘要；无则新建文档，并把一行摘要登记进知识索引。
- 详情页与 Story 领域知识 → 维护 `/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/业务模块/详情页-领域地图.md` 与 `/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/业务模块/Story-领域地图.md`（有内容时才创建，并登记进知识索引）：模块职责、数据流与归属、近期改动与结论、常见坑、验收要点。
- 可复用模板/标准写法 → `/Users/sgx/Documents/Notes/4-Agents/plugins/sgx-skills/template`（先查重）。

**待确认（写入待确认清单，不进正式区）：**

- 行为纪律/协作约束 → 候选去向 `/Users/sgx/Documents/Notes/4-Agents/memory/knowledge/规则与纪律/`
...
- 跨项目通用偏好、多次验证的决策 → `/Users/sgx/Documents/Notes/4-Agents/memory/habits`、`decisions`
...
- `/Users/sgx/Documents/Notes/4-Agents/memory/daily/YYYY-MM-DD.md` 汇总：A 的任务变化；B 的沉淀结果（直写 n 条列文件 / 候选 n 条 / 跳过 n 条及原因）；C 的待确认提醒。
- 更新 `/Users/sgx/Documents/Notes/4-Agents/memory/projects/` 对应页；30 天无变化任务按 archive 规则处理。
- 更新 `/Users/sgx/Documents/Notes/4-Agents/memory/同步状态.md`：last_run、last_commit（本次运行结束时的 HEAD）、处理过的 task_id、每日文件链接、运行结果。
