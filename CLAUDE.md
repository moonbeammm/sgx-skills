...
- 每个任务开始先列目标、允许修改路径、明确不做项、完成条件和预算；默认 20 分钟或 10 次工具调用，先到者停止并报告。每 5 次工具调用或 10 分钟汇报一次。
- 需要扩大范围、删除/迁移、改自动任务、调用外部系统或预计超预算时先取得用户确认。网络/依赖/模型失败最多重试 1 次，之后标记未验证并交付已有结果。
- 后台子 agent 最多 1 个、最长等待 5 分钟，不递归派发。达到完成条件立即停止，最终分列已改、未改、已验证、未验证。
